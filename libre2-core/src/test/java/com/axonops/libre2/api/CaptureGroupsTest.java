...
class CaptureGroupsTest {
    ...
    void matchResult_successfulMatch_matched() {
        ...
        MatchResult result = pattern.match("123");
    ...
    void matchResult_failedMatch_notMatched() {
        Pattern pattern = Pattern.compile("(\\d+)");
        MatchResult result = pattern.match("abc");
    ...
    void patternMatch_partialContent_fails() {
        Pattern pattern = Pattern.compile("(\\d+)");
        MatchResult result = pattern.match("abc123def");
    ...
    void patternFind_firstMatch_found() {
        Pattern pattern = Pattern.compile("(\\d+)");
        MatchResult result = pattern.find("abc123def456");
    ...
    void patternFind_firstMatchGroups_extracted() {
        Pattern pattern = Pattern.compile("([a-z]+)@([a-z]+\\.[a-z]+)");
        MatchResult result = pattern.find("Contact support@example.com or admin@test.org");
    ...
    void patternFind_notFound_failedMatch() {
        Pattern pattern = Pattern.compile("(\\d+)");
        MatchResult result = pattern.find("no digits here");
    ...
    void optionalGroups_notParticipating_null() {
        ...
        MatchResult result = pattern.match("b");  // 'a' is optional and doesn't match
    ...
    void nestedGroups_extracted() {
        ...
        MatchResult result = pattern.match("123-456");
    ...
    void realWorld_emailExtraction() {
        ...
        MatchResult result = pattern.match("john.doe@example.co.uk");
    ...
    void realWorld_logParsing() {
        ...
        MatchResult result = pattern.find("[1234567890] ERROR: Something went wrong");
    ...
    void match_consistentWithMatches(String patternStr, String input, boolean shouldMatch) {
        ...
        MatchResult matchResult = pattern.match(input);
    ...
    void find_consistentWithMatcher() {
        ...
        MatchResult findResult = pattern.find("abc123def");
    ...
    void emptyString_works() {
        ...
        MatchResult result = pattern.match("");
    ...
    void nullInput_throws() {
        ...
        assertThatNullPointerException()
            .isThrownBy(() -> pattern.match(null));

        assertThatNullPointerException()
            .isThrownBy(() -> pattern.find((String) null));  // Cast to disambiguate

        assertThatNullPointerException()
            .isThrownBy(() -> pattern.findAll(null));
    }
}
